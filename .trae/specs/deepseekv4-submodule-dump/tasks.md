...
- [x] Task 1: 在 `deepseek_v4.py` 中新增 `_moe_dump_sub` 辅助函数
...
- [x] Task 2: 在 `dsa_v1.py` AscendDSAImpl `_forward_prefill` 中添加 dump
...
- [x] Task 3: 在 `dsa_v1.py` AscendDSAImpl `_forward_decode` 中添加 dump
  - 同 Task 2 的 prefill 逻辑，添加相同算子路径
  - 额外添加 `attention_q_a_layernorm/input.pt` 和 `attention_q_a_layernorm/output.pt`（decode 动态量化路径）

## Compressor 子模块 dump

- [x] Task 4: 在 `dsa_v1.py` AscendDSAImpl `_forward_prefill` 和 `_forward_decode` 中添加 Compressor dump
...
- [x] Task 5: 添加 scatter 子模块 dump
  - `scatter_cmp_kv/input.pt` 和 `scatter_cmp_kv/output.pt`

## DeepseekV4Gate 子模块 dump

- [x] Task 6: 在 `deepseek_v4.py` DeepseekV4MoE.forward 中添加 dump
...
- [x] Task 7: 在 `dsa_v1.py` indexer_select_qli 中添加 dump
...
- [x] Task 8: 在 `fused_moe.py` AscendSharedFusedMoE.forward_impl 中添加 dump
...
- [x] Task 9: 在 `fused_moe.py` AscendUnquantizedFusedMoEMethod.apply 中添加 dump
...
- [x] Task 10: 在 `experts_selector.py` select_experts 中添加 dump
...
- [x] Task 11: 在 `moe_mlp.py` apply_mlp 中添加 dump
...
- [x] Task 12: 在 `w8a8_dynamic.py` AscendW8A8DynamicFusedMoEMethod.apply 中添加 dump
...
- [x] Task 13: 在 DeepSeekV4 mHC 相关操作中添加 dump
